...
repository and not admitted, the battery is **not complete**, no official campaign has been completed or
admitted (its diagnostic measurements are not accepted campaign results or native
performance evidence), and the resource-sampler defect is addressed under its own change,
...
**Statements of the amendment of 2026-09-18 superseded** *(recorded 2026-09-19)*. The paragraph of facts and rule 1 of that amendment stood on `dev` and are kept as written; three of their statements are superseded. The first-flow evidence they call unsealed is a locally hash-sealed candidate (all 51 `SHA256SUMS` entries verify), outside the published evidence package and not admitted. The corrective work the paragraph of facts calls open — 768 MiB for the three Ditto services and a graceful stop before power-off — has since been merged into `dev` through pull request #34; only bounded observations follow from it, and no general or prolonged stability is claimed. And "nothing has been measured" is too broad: no official campaign has been completed or admitted, and the diagnostic measurements are not accepted campaign results or native performance evidence.
