...
**Date:** 2026-09-16; amended 2026-09-18 (evidence classes and proposal status, see the last section); links and facts updated 2026-09-18 when the plan v2.0 proposal moved under `docs/governance/proposals/`
...
**Authority:** the student's explicit instruction, for technical planning only and not supervisor approval, as set out in the [integrated plan v2.0 proposal](../governance/proposals/INTEGRATED_DEVELOPMENT_PLAN_2026_v2.0_proposal.md), itself a proposal kept under `docs/governance/proposals/` and not in force; [plan v1.2](../governance/INTEGRATED_DEVELOPMENT_PLAN_2026.md), at the canonical path, remains the plan in force until the student decides after consulting the supervisors.
...
This amendment changes no part of the decision above; it fixes how evidence is classed for emulated and native runs. It matches [section 10 of the plan v2.0 proposal](../governance/proposals/INTEGRATED_DEVELOPMENT_PLAN_2026_v2.0_proposal.md#10-evidence-classes-emulated-qemutcg-versus-native-arm64).

**Facts.** On 2026-09-17 no native ARM64 virtual machine had been obtained (the student's attempts with cloud providers had not produced one). The project review therefore ordered an integrated QEMU/TCG profile first: the Yocto ARM64 guest emulated on the x86-64 WSL2 workstation, with the six containers inside the guest; the native ARM64 route is the subsequent step. On 2026-09-18, in pull request #28 (merged into `dev` on 2026-09-18), the integrated image was built (commit `03e333e`) and booted twice under QEMU/TCG (commit `3209b17`) with every build and boot acceptance check passing. The same day an isolated MongoDB 7.0.39 test passed on that guest (start, write/read, restart, persistence across a guest power cycle); the build, boot and MongoDB records were sealed as technical evidence in `docs/evidence/integrated-qemu/` (pull request #29, merged into `dev` on 2026-09-18). Pull requests #32 (hand-over of the broker secrets and the ACL probe) and #31 (progress counters of the controller) were merged into `dev` the same day; merging validates no stack and accepts no gate. The six-container stack has not been deployed, nothing has been measured, sealing is not acceptance, and no gate and no claim is accepted. The first bounded end-to-end functional test (one smartwatch, 1 Hz, 60 s) was authorised on 2026-09-18 and is in preparation; it has not run. The audit of the sealed G1 image, `docs/reviews/2026-09-17-egw-image-audit.md`, and the runbook `docs/setup/qemu_integrated_gateway.md` belong to pull request #28.

**Rules.**

1. *What runs under QEMU/TCG (ARM64 emulated on x86-64) may demonstrate.* Build and redeployment of the versioned Yocto image, boot, the container runtime, deployment of the six-container stack inside the guest, the functional path smartwatch simulator -> MQTT/TLS -> controller -> Ditto -> API, correctness, fault handling and recovery, and persistence. This is functional and integration evidence only; of the list, only the build, the boot and the isolated MongoDB test have been demonstrated so far. Timing observed under emulation may be recorded only as informational and must be labelled emulated.
