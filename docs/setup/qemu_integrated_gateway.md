...
```bash
host$ scp "egw-tcg:/opt/egw/deployment/data/events/$RID.warmup/events.jsonl" $P/$RID.warmup.events.jsonl && [ -s $P/$RID.warmup.events.jsonl ] && $REC delta ~/egw-tcg/pilot/results/raw/$RID --prefix $P/$RID --events $RAW6/events.post-drain.jsonl --also $P/$RID.warmup.events.jsonl || stop "test 6: the warm-up log was NOT fetched (delta was then not run), or delta exited non-zero (4 = MISMATCH)"
```
