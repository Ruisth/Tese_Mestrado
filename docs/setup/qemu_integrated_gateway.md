...
# The reload count comes only from a log the guest read (docker compose logs exit 0, output not empty): a failed
# or empty read stops with no file, because a count of zero over a log that was not read would state "no
# reload" about what was not observed (review of 2026-09-25, D2).
config_identity() {
  local out=$1 raw rc
...
log=$(docker compose --env-file .env --env-file images.lock.env logs --no-color mosquitto); lrc=$?
if [ "$lrc" != 0 ] || [ -z "$log" ]; then echo "config_identity: the broker log was NOT read on the guest (docker compose logs exit $lrc, ${#log} characters): whether the broker reloaded is neither observed nor excluded" >&2; exit 5; fi
echo "reloaded=$(printf "%s" "$log" | grep -c "Reloading config")"
...
echo "paho=$(docker exec egw-controller-1 python -c "import importlib.metadata as m; print(m.version(\"paho-mqtt\"))")"'); rc=$?
  case $rc in
    0) ;;
    5) stop "config_identity: the broker log was NOT read on the guest (its line above says why): whether the broker reloaded is not observed, and no identity is written without that observation"; return 1;;
    *) stop "config_identity: ssh egw-tcg exited $rc - nothing was written"; return 1;;
  esac
  printf '%s\n' "$raw" | python3 -c '
import json, re, sys
out = sys.argv[1]
