...
**Status (2026-09-18):** Sections 1 to 3.4 were executed once on 2026-09-18: the image was built from commit `03e333e` and booted twice from commit `3209b17` (which changes only the boot wrapper and this runbook); every acceptance check of 2.4 and 3.4 passed, the data disk and the journal persisted across the two boots, and the candidate evidence (logs, checksums, guest transcripts, with `SHA256SUMS`) is held outside the repository under `~/yocto/evidence-candidates/2026-09-18-integrated-{68f9ae7,03e333e,3209b17}/` in the WSL2 home and is **not sealed** (`docs/reviews/2026-09-17-egw-image-audit.md`, Section 13). That run used the commands of Sections 2-3 as they stood before the guard rewrite of the same day: the compound guards of 3.1 and 3.3, and every guard and helper of Sections 4-9, have **not** been executed on the real host or guest, and the `UNVERIFIED:` marks of Sections 1-3 were not revisited against the run (Appendix B items 20 and 22). **Nothing from Section 3.5 onwards has been run**: no image archive was loaded, no container was started, no traffic reached port 8883. Every command is to be executed by the student in WSL2 (Ubuntu-24.04, user `ruisth`) or on the Windows host as stated; every "expected" value is a prediction to be confirmed or refuted by the evidence the step produces. Where a step depends on a design decision of the integrated profile that is not yet fixed, the assumption is stated in the step and collected in Appendix A. Anything not verified against the pinned checkout or the repository is marked `UNVERIFIED:` and listed in Appendix B.
...
```bash
host$ if ss -ltn | grep -E ':(8000|8080) '; then echo "STOP: host port busy (line above) - the tunnel was NOT opened; a stale tunnel is closed with: pkill -f 'ssh -f -N'"; else ssh -f -N -o ExitOnForwardFailure=yes -L 8000:127.0.0.1:8000 -L 8080:127.0.0.1:8080 egw-tcg && echo "TUNNEL UP" || echo "STOP: ssh exited non-zero - the tunnel was NOT opened"; fi
...
# drained: no work queued AND none in progress, as far as GET /metrics can show it (Section 7, "What drained proves").
# EVERY reading taken during one unbroken quiet window of DRAIN_QUIET_S seconds must show queue_depth 0 and the
# same started_at and counters. A non-zero queue, any counter movement, or a controller restart opens a new window.
# DRAIN_QUIET_S must stay ABOVE the NOMINAL longest time one message can be in progress (130 s > 122.4 s with the
# default retry settings). That figure is NOT a proven bound (Section 7 (b)): it assumes that one 10 s timeout phase
# dominates each attempt; the per-phase sum is 40 s per attempt, 482.4 s for a first-contact message - export
# DRAIN_QUIET_S=490 to cover it. Never lower it. 'drained' INFERS that nothing is in progress; 'accounted' (below)
# checks it positively for the messages of one run.
drained() {
  ...
  while :; do
    ...
    if [ "${cur%% *}" != 0 ] || [ "$cur" != "$ref" ]; then
      ...
      if [ $((SECONDS - since)) -ge "$quiet" ]; then
        echo "drained: queue_depth 0 and identical counters on $n consecutive readings over $((SECONDS - since)) s ($cur)"
        return 0
      fi
    fi
...
# accounted <run-id>: POSITIVE check that nothing of THIS run is still in progress (Section 7, "What drained proves").
# The host knows what was published (sent_events.jsonl), so the left-hand side that /metrics lacks is available here:
# N = published records; M = movement of accepted+rejected+duplicate+failed+dropped since <run-id>.metrics.before.json
# (same controller process). A counter moves only when the processing of a message has ENDED, and there is a single
# consumer, so M >= N means that every published message has ended. M < N is a STOP: N-M messages are still in
# progress, ended without a counter, or never reached the controller. The last two are permanent and waiting cannot
# cure them, so this function does not loop. ACCEPT_UNACCOUNTED=1 lets the capture go on, records the fact in
# <run-id>.unaccounted.txt and makes 'finish' end non-zero. The fetched events.jsonl is used only for the informative
# figure without_event_record (published message_ids with no record in this run's log).
accounted() {
  local id=$1 now rc
  [ -s "$P/$id/sent_events.jsonl" ] && [ -s "$P/$id/events.jsonl" ] && [ -s "$P/$id.metrics.before.json" ] \
    || { stop "accounted $id: sent_events.jsonl, the fetched events.jsonl or metrics.before.json is missing or empty - nothing to account against"; return 1; }
...
logged = {e.get("message_id") for e in jl(run_dir + "/events.jsonl")}
b = json.load(open(before_path, encoding="utf-8"))
a = json.loads(now_json)
keys = ("accepted", "rejected", "duplicate", "failed", "dropped")
n = len(sent)
u = sum(1 for r in sent if r.get("message_id") not in logged)
head = "IN-PROGRESS CHECK %s: sent=%d without_event_record=%d" % (rid, n, u)
if a["queue_depth"] != 0:
    print(head, "queue_depth=%s -> the controller is not drained" % a["queue_depth"]); sys.exit(1)
if a["started_at"] != b["started_at"]:
    print(head, "-> NOT EVALUABLE: the controller process restarted since the before reading (started_at differs) and its counters restarted from zero; only the quiet window of drained supports the after snapshots"); sys.exit(0)
d = {k: a[k] - b[k] for k in keys}
m = sum(d.values())
head += " terminal=%d %s" % (m, d)
if m >= n:
    extra = "" if m == n else " (terminal exceeds sent by %d: QoS 1 redelivery or another publisher; the check is then necessary, not sufficient)" % (m - n)
    print(head, "-> OK: every published message has reached a terminal counter" + extra); sys.exit(0)
if accept == "1":
    print(head, "-> UNACCOUNTED=%d, accepted by the operator (ACCEPT_UNACCOUNTED=1): report as measured" % (n - m)); sys.exit(5)
print(head, "-> UNACCOUNTED=%d: still in progress, ended without a counter, or never reached the controller" % (n - m)); sys.exit(1)
' "$id" "$P/$id" "$P/$id.metrics.before.json" "$now" "${ACCEPT_UNACCOUNTED:-0}"; rc=$?
  case $rc in
    0) return 0;;
    5) echo "$(date -u +%FT%TZ) accounted $id: unaccounted messages accepted by the operator (see the IN-PROGRESS CHECK line)" >> "$P/$id.unaccounted.txt"; return 0;;
    *) stop "accounted $id: not every published message has reached a terminal counter (line above). Repeat 'finish $id' later; if the figure does not change it is permanent: 'ACCEPT_UNACCOUNTED=1 finish $id' captures the state and reports the run as FAILED"; return 1;;
  esac
}
...
finish() {
  ...
  [ ! -e "$P/$id.unaccounted.txt" ] || { stop "finish $id: captured, but with unaccounted messages accepted by the operator ($P/$id.unaccounted.txt) - report as measured, not as complete"; return 1; }
}
...
# failure: the marker and the evidence are still needed), then one status line. Non-zero if either failed.
sim_post() {
  local id=$1 sim_rc post_rc; shift
  $SIM "$@" --run-id "$id" 2>&1 | tee "$P/$id.stderr.txt"; sim_rc=${PIPESTATUS[0]}
  post "$id"; post_rc=$?
  if [ "$sim_rc" = 0 ] && [ "$post_rc" = 0 ]; then
    echo "TEST STATUS $id: simulator exit=0 post=0 -> PROCEDURE COMPLETE. This is NOT the verdict: compare the values printed above with the test's Expected list."
    return 0
  fi
  stop "TEST STATUS $id: simulator exit=$sim_rc post=$post_rc -> FAILED. Do not start another run before the cause is understood."
}
...
| `drained` | proves, as far as `/metrics` allows, that nothing is queued **and** nothing is in progress (Section 7, "What `drained` proves") | `/metrics` unreachable or not JSON; no quiet window of `DRAIN_QUIET_S` (130 s) within `DRAIN_LIMIT_S` (900 s) |
...
| `accounted <run-id>` | positive in-progress check for one run: published records (`sent_events.jsonl`) against the movement of the five counters since `metrics.before.json`; prints one `IN-PROGRESS CHECK` line (Section 7) | fewer terminal outcomes than published messages; `queue_depth` not 0; `/metrics` unreachable; an input file missing. Not evaluable (reported, returns 0) when `started_at` changed. `ACCEPT_UNACCOUNTED=1` continues, records `<run-id>.unaccounted.txt` and makes `finish` end non-zero |
...
| `finish <run-id>` | `wait && drained && fetch && accounted && snap_pair after`, then `check` **and** `delta` (both always run) | any capture step fails, `check` exits non-zero (3 = not a protocol check) or `delta` exits non-zero (4 = `MISMATCH` or queue not empty), or unaccounted messages were accepted by the operator; repeatable after the cause is fixed — an existing `after` pair is kept, never retaken; a lone `after` file is a `STOP` |
| `sim_post <run-id> <sim args>` | simulator (stderr teed to `<run-id>.stderr.txt`), then `post` immediately **whatever the simulator's exit status**, then one `TEST STATUS` line | simulator exit ≠ 0 or `post` ≠ 0 — the marker and the evidence are still collected |
...
`$REC wait` returns when the **controller** clock has passed marker + 60 s (it replaces a host-side sleep). The closed window says nothing about work still queued or in progress — under TCG a backlog can outlive it — so the `after` snapshots are taken only once `drained` has also succeeded **and** `accounted` has confirmed, from the fetched log's run and the counters, that every published message of this run has reached a terminal counter (Section 7, "What `drained` proves"; `fetch` therefore moved from 6.4 to this line, and the `[ -f ...window-closed.json ] ||` lets the line be repeated after a `STOP` without a second, refused, `wait`). `snap_pair` writes `$P/$RUN.metrics.after.json` and the `after` twin snapshot together, both write-once. The confirmation deadline is unaffected, because `check` judges every record against the marker deadline, not against the time of the snapshot.
...
```bash
host$ if [ -s $P/$RUN.metrics.after.json ] && [ -s $P/$RUN.twins.after.json ] && [ -s $P/$RUN/events.jsonl ]; then $REC check $P/$RUN; C=$?; $REC delta $P/$RUN; D=$?; echo "FLOW STATUS $RUN: check exit=$C delta exit=$D (both must be 0; then compare the printed values with the acceptance below)"; [ "$C" = 0 ] && [ "$D" = 0 ] || stop "6.4: check or delta exited non-zero"; else echo "STOP: 6.3 did not complete (no 'after' pair or no fetched events.jsonl) - check and delta were NOT run"; fi
```

The guard requires what only a completed 6.3 leaves behind (the `after` pair and the fetched `events.jsonl`): the first version ran `check` whenever `fetch` succeeded, also after a `STOP` in 6.3, and `delta` then ended in a Python traceback instead of a `STOP` line. `check` refuses a log fetched before the window closed. `check` is the harness accounting (`egw_experiments.analyze.compute_run_metrics`) with the marker deadline; `delta` is the per-device twin delta and the per-process `/metrics` delta against `events.jsonl`. Both always run, and both exit codes are shown on one line.
...
host$ wait_ready 3600 && $REC snap --prefix $P/$RUN --label post-restart --like after && $REC same --prefix $P/$RUN after post-restart || stop "6.5: persistence NOT verified (stack not ready, snapshot failed, or 'same' reported DIFFERENT)"
...
host$ cp ~/egw-tcg/sut_environment.json ~/egw-tcg/itest-helpers.sh ~/egw-images/archive-identity.txt ~/egw-images/SHA256SUMS $EVR/     # the helper file holds no secret (6.1)
...
2. `post` runs immediately after the simulator **even when the simulator exits non-zero** — the marker and the evidence of a failed run are still needed — but the status is then non-zero and the line reads `TEST STATUS <id>: simulator exit=<n> post=<m> -> FAILED`.
...
**What `drained` proves, and what it cannot.** Both `pre` (before the `before` snapshots) and `finish` (before the fetch and the `after` snapshots) call `drained`. Without it, a backlog of the previous run that outlives its 60 s window under TCG would be logged to the previous run's `events.jsonl` but counted in this run's Δ`accepted_count` and `/metrics` differences: a false `MISMATCH` (never a false `OK`), since `delta` itself checks `queue_depth` only in the `to` snapshot. What `GET /metrics` offers for this was established from the controller sources (read on 2026-09-18; nothing was run):
...
Since the code supports no instantaneous invariant, `drained` uses the most conservative procedure available without changing the controller: a **quiet window longer than the nominal longest time one message can be in progress**. Nominally — assuming that one 10 s timeout phase dominates each attempt, see (b) below — one logical Ditto request is at most `EGW_RETRY_MAX` attempts of 10 s each (`DEFAULT_TIMEOUT_S`, `ditto.py` line 24; `from_settings` passes no other timeout, lines 170-187) plus the back-off sleeps `EGW_RETRY_BACKOFF_MS` × (2^(n−1)) between them (`ditto.py` lines 202-203 and 228-250): with the defaults 3 and 200 ms (`config.py` lines 75-76; `.env.example` lines 63-64) that is 3 × 10 + 0.2 + 0.4 = 30.6 s. One message issues at most four logical requests — first contact: `get_twin`, then policy `PUT` and thing `PUT` in `ensure_twin`, then the `PATCH` (`service.py` lines 269-271, 299 and 357-364; `ditto.py` lines 311-320) — hence at most 4 × 30.6 = **122.4 s**; any other message needs at most 30.6 s. `drained` therefore returns 0 only when **every** reading (one about every 5 s, some two dozen in all) over an unbroken window of `DRAIN_QUIET_S` = 130 s shows `queue_depth` 0 and identical `started_at`, `accepted`, `rejected`, `duplicate`, `failed` and `dropped`; a non-zero queue, any counter movement or a new `started_at` opens a new window; an unreachable or non-JSON `/metrics` is a `STOP`, never a reading; without a quiet window within `DRAIN_LIMIT_S` = 900 s it gives up with `STOP` (a sizing observation for Section 8) and the run must not be started. Why this covers work in progress: a message that was in progress at the first reading of the window ends — nominally — within 122.4 s, and it ends either in a counter (the window is then broken and restarts) or in the swallowed-exception path (no counter, but no work in progress any more either — it then shows up as a `MISMATCH` in `delta`, not as a silent pass). If `EGW_RETRY_MAX` or `EGW_RETRY_BACKOFF_MS` is changed, recompute 4 × (R × 10 + B/1000 × (2^(R−1) − 1)) and export a larger `DRAIN_QUIET_S`; never a smaller one. Cost: at least 130 s before and after every ad-hoc run.

What the procedure still **cannot** see (`UNVERIFIED:`, Appendix B item 21): (a) a message delivered to the controller during the last seconds of the window and still in progress at its last reading — it was never in a reading's `queue_depth` and has not reached a counter; this is excluded only by the operator's rule that no simulator, harness run or probe is publishing while `drained` runs, together with the bridge not asking the broker for a persistent session (`mqtt.py` lines 73-76 pass no `clean_session`; the paho-mqtt default is library behaviour, not checked in the installed wheel); (b) **122.4 s is a nominal figure, not a bound.** `httpx.AsyncClient(timeout=10.0)` (`ditto.py` lines 165-167) sets the connect, write, read and pool timeouts to 10 s **each**, and the phases of one attempt are sequential: with no misbehaviour at all, an attempt that spends almost 10 s connecting and almost 10 s waiting for the response lasts about 20 s, and the per-phase sum is 40 s, i.e. 3 × 40 + 0.6 = 120.6 s per logical request and 4 × 120.6 = **482.4 s** for a first-contact message — above the default `DRAIN_QUIET_S`. On top of that the read timeout applies per read, so a response that trickles in can keep one attempt alive longer still. The default stays at 130 s because a window of 490 s before and after every ad-hoc run would add about 12 minutes to each of them, because the failure it guards against is conservative (a message of the previous run still in progress at the `before` snapshot produces a false `MISMATCH`, never a false `OK`), and because `finish` no longer rests on the window alone (`accounted`, below); `export DRAIN_QUIET_S=490` covers the per-phase sum whenever a `MISMATCH` has to be excluded as a timing artefact; (c) work inside Ditto or MongoDB after Ditto's 2xx is outside the controller's view altogether (the twin snapshots read Ditto's own API, which is the state the tests assert). The instantaneous check the project review asked for needs the controller change described in Section 9 item 7 (not implemented: `src/egw_controller` is unchanged); until then, **in `pre`** this window is the strongest statement available — absence of work in progress is there *inferred*, not observed, since no count of published messages exists for whatever ran before. `UNVERIFIED:` never executed against the real controller (stubbed `curl` only, Appendix B item 20).

**The positive check in `finish`: `accounted`.** After a run the inference can be replaced by an observation that needs no controller change, because the host knows the left-hand side of the invariant for this run: `sent_events.jsonl` holds one record per published message (N). `accounted <run-id>` (6.1) reads `/metrics` once more after `drained` and `fetch`, and compares N with M, the movement of `accepted + rejected + duplicate + failed + dropped` since `<run-id>.metrics.before.json` (only when `started_at` is the same; otherwise it prints `NOT EVALUABLE` and the window alone supports the snapshots, as in a controller-restart run). A counter moves only when the processing of a message has ended (`_emit`, `service.py` lines 393-394; `dropped` at enqueue time, lines 156-159) and there is a single consumer, so **M ≥ N is direct evidence that every published message has ended** — no timing argument is involved. It prints one line, `IN-PROGRESS CHECK <run-id>: sent=N without_event_record=U terminal=M {...} -> OK | UNACCOUNTED=k`, and only `OK` (or `NOT EVALUABLE`) lets `finish` take the `after` snapshots. Limits, stated rather than hidden: (i) M > N happens with QoS 1 redelivery (test 5) — each redelivered copy adds a `duplicate` — and the surplus could then mask the same number of messages still in progress, so with M > N the check is necessary, not sufficient, and the line says so; `without_event_record` (published `message_id`s with no record at all in the fetched log) is printed as the per-message figure to read next to it: it should equal the `dropped` movement, except for payloads rejected before their identity could be read (`_extract_identity` is best-effort, and a record without a usable `run_id` goes to the `unknown` bucket, `events.py` lines 36 and 75-82); (ii) M counts every message the controller handled since the `before` reading, so the operator's rule that nothing else publishes is still assumed; (iii) M < N is not always work in progress: a message swallowed without a counter (`service.py` lines 177-181), dropped before the bridge had its loop, or never delivered by the broker leaves M < N **for ever**. `accounted` therefore reports and stops, it does not loop: repeat `finish <run-id>` once later; if the figure is unchanged the shortfall is permanent, and `ACCEPT_UNACCOUNTED=1 finish <run-id>` captures the `after` state, records the decision in `<run-id>.unaccounted.txt` and still ends non-zero (`check` reports the same messages as `lost`). `UNVERIFIED:` never executed against the real controller (stubs only, Appendix B items 20 and 21).
...
host$ if [ "$RT" = 0 ]; then for U in $(python3 -c "import json;print(' '.join(sorted({json.loads(l)['device_uuid'] for l in open('$HOME/egw-tcg/itest/$R/sent_events.jsonl')})))"); do twin $R $U || break; done; else stop "test 2: run_test did not complete (RT='$RT') - the twins were NOT read"; fi
...
host$ if [ "$RT" = 0 ]; then ssh egw-tcg "cd /opt/egw/deployment && docker compose --env-file .env --env-file images.offline.env logs --no-color mosquitto | grep -E 'egw-simulator-$R'" > ~/egw-tcg/itest/$R.broker.txt; wc -l ~/egw-tcg/itest/$R.broker.txt; else stop "test 5: broker log NOT read (RT='$RT')"; fi
```

Expected: stderr `dropout_disconnects = N` with N ≈ 3 (one window per minute, `dropout_windows`), `buffered_dropout > 0`; the manifest carries the `DROPOUT_SCOPE_NOTE`; the broker log shows N+1 connections and N disconnections for the client id `egw-simulator-<run_id>` (`UNVERIFIED:` exact Mosquitto wording — "New client connected from ... as egw-simulator-..." and "Client egw-simulator-... disconnected"/"closed its connection"); `lost = 0` and `late_confirmations = 0` under the controller-clock deadline (buffered events are published late but must be confirmed inside the window), and every `delta` line `OK`. The `IN-PROGRESS CHECK` line of this test may show `terminal` above `sent` by the number of redelivered copies (`duplicate` movement); it is then a necessary check only (Section 7, `accounted`, limit (i)). If `lost > 0`, `late_confirmations` says directly how many accepted records were confirmed after the deadline — a guest-speed finding under TCG, reported as measured; `duplicates` (QoS 1 redelivery after reconnect) are not failures, `double_accepted` must be 0.
...
host$ pkill -f 'ssh -f -N' ; ssh -f -N -o ExitOnForwardFailure=yes -L 8000:127.0.0.1:8000 -L 8080:127.0.0.1:8080 egw-tcg && echo "TUNNEL UP" || stop "test 8: tunnel NOT opened"
host$ wait_ready 3600 && $REC snap --prefix $P/itest-reboot --label post-reboot --like pre-reboot && $REC same --prefix $P/itest-reboot pre-reboot post-reboot || stop "test 8: persistence NOT verified (stack not ready, snapshot failed, or 'same' reported DIFFERENT)"
