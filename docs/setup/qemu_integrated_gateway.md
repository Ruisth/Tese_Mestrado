...
Expected: `manifest.json` with `validity: "valid"`, `SHA256SUMS` written, `resources.csv` with the `host` column equal to the guest hostname (the harness rejects it otherwise — `collect-resources.sh` header), `controller_metrics.csv`, `sut_environment.json` copied in. `sudo systemd-run` is used because the collector must outlive the SSH session and stop cleanly on `systemctl stop`; `egw` has NOPASSWD sudo (assumption A13). A 30 s smoke satisfies the collector's minimum of 30 distinct instants only just (**superseded on 2026-09-19**, kept where it stands: the first cgroup sample of a fresh state file only primes the CPU delta and writes no row, so the collector now needs to run one interval longer than the window it has to cover — started before the warm-up and stopped after the confirmation window, as the hooks do, that interval falls outside the measured window, but a 30 s smoke no longer has a spare instant if the collector is started with the window); if the run is marked invalid for coverage, repeat with `--run-id` of a nominal entry and `--duration` untouched (the plan fixes durations). The harness applies the confirmation deadline itself (manifest `confirmation_deadline_clock_domain: "controller"`), so no `pre`/`post` is used here; read the result with `python -m egw_experiments analyze --base-dir ~/egw-tcg/pilot/results --plan ~/egw-tcg/pilot/campaign_plan.json` and the columns `confirmation_deadline_source`, `sent_valid`, `delivered_unique`, `lost`, `late_confirmations`, `double_accepted` of `~/egw-tcg/pilot/results/processed/per_run.csv` (acceptance rows of conditions that were not run are expected to fail on completeness in a pilot tree).

**The resource collector, and why this harness part failed on 2026-09-18 (note of 2026-09-19).** On 2026-09-18 the harness parts of this test and of test 6 failed on the resource series, and the harness was right to fail them: each `docker stats --no-stream` call cost 3 to 5 s idle and about 21 s while the stack was working under TCG, so the collector produced 7 distinct sample instants across 42 rows in a 150 s window against the ingest minima of 30 rows and 30 distinct instants (`MIN_RESOURCE_SAMPLES`, `MIN_DISTINCT_SAMPLE_INSTANTS` in `src/egw_experiments/resources.py`); the run was marked `invalid` and `SHA256SUMS` was withheld. Those figures were recorded in a candidate-evidence note held outside this repository and unsealed — they are not evidence here. The sampler is the part that changed on 2026-09-19: `collect-resources.sh` now reads the guest's cgroup v2 accounting under `/sys/fs/cgroup` instead of calling the Docker daemon (`src/deployment/README.md`, section "SUT-side evidence collection"), the columns keep their meaning, and `--source docker` restores the previous docker-stats loop for a host whose cgroup tree is not visible. **The harness parts of tests 1 and 6 have not been re-run since, so the battery is not complete, nothing is measured and no gate is closed;** the re-run is the acceptance step. The three collector hooks of `harness_run` (6.1) are unchanged and remain correct: the collector takes its default source (`auto`) with no `--source` argument, as it did before.

Read which source the collector actually used. The choice is made once, before the first sample, and the collector announces it only on stderr, which under `systemd-run` goes to the guest journal (persistent, assumption A14) and never into the CSV — a run whose journal line was not kept cannot be shown afterwards to have sampled from the cgroup tree:

```bash
host$ if ssh egw-tcg "sudo journalctl -u egw-resources-$RID --no-pager" > $P/$RID.collector.txt && [ -s $P/$RID.collector.txt ] && grep -E 'collecting container resources from (cgroup|docker)' $P/$RID.collector.txt; then echo "COLLECTOR SOURCE RECORDED"; else stop "test 1: the collector's source line was NOT captured - the resource series of $RID cannot be attributed to a sampling source"; fi     # 'from cgroup' is the new default; 'note: no container cgroup found ...' followed by 'from docker' means the run fell back to the slow path and its series is expected to be short
```

`UNVERIFIED:` that `journalctl -u egw-resources-<run_id>` still holds the line after `systemd-run --collect` has garbage-collected the unit, and everything else in this note that concerns the guest: the cgroup path has never run there (Appendix B item 23).
...
**Note of 2026-09-19 — the sampler, and what this test still depends on.** The harness part of this test failed on 2026-09-18 for the same reason as test 1 (the collector's `docker stats` calls were too slow under TCG to reach the ingest minima; see the collector note under test 1). The sampler was changed on 2026-09-19 and **this test has not been re-run**, so nothing here is measured. Two things to expect when it is: the restarted controller keeps its container id while its cgroup disappears and reappears, so the collector re-primes that container and writes no row for it while it is down; and `validate_resources_csv` measures sampling gaps **per container** (`resources.py`), so a controller that takes more than about 4 s to come back under TCG leaves a hole above `MAX_SAMPLE_GAP_S = 5.0` and the run is marked invalid for behaviour this test deliberately provokes. That is pre-existing, it was equally true of the docker-stats collector, and it means the re-run of test 6 must not be presented as gated only on the sampler.
...
4. **Resource competition.** QEMU/TCG (4 vCPU threads plus I/O and the display-less console), the simulator, the harness sampler and the SSH tunnels all share the WSL2 VM's 16 CPUs and its memory limit; container `cpu_pct` from `docker stats` inside the guest is relative to the *emulated* CPUs, and the QEMU process's host utilisation is a different quantity — document both (Section 9). (**The naming of the mechanism is superseded on 2026-09-19**, the statement is not: since that date `cpu_pct` comes from the guest's cgroup v2 accounting (`cpu.stat usage_usec` over wall clock) and not from `docker stats`, on the same single-CPU basis and still relative to the *emulated* CPUs.) No Yocto build, no Windows sleep/hibernate, no Docker Desktop builds during a pilot run.
...
5. **Collectors** — `collect-resources.sh` unchanged (it already stamps `host`); add an optional host-side sampler for the QEMU process (`top -b -n 1 -p <pid>` at 1 Hz on the WSL host into `qemu_process.csv`) so emulated-CPU utilisation and host utilisation are never confused. (**"unchanged" is superseded on 2026-09-19**: the collector was changed that day because its `docker stats` calls could not sustain the cadence under TCG — it now reads the guest's cgroup v2 accounting by default, keeps the columns and the `host` stamp, and offers `--source docker` for a host whose cgroup tree is not visible; test 1's collector note and `src/deployment/README.md` describe it. The host-side QEMU-process sampler is still not implemented. What this item asks for is otherwise unchanged, and the new path is unproven on the guest — Appendix B item 23.)
...
23. **The cgroup v2 sampling path of `collect-resources.sh` on the guest (change of 2026-09-19; test 1's collector note, Section 8 item 4, Section 9 item 5).** The path is covered by `src/tests/test_collect_resources.py` against a synthetic cgroup tree — 11 scenarios under `sh`, `dash` and `bash`, `33 passed` on WSL2 Ubuntu-24.04 — and by one smoke run against the workstation's own tree, which had no container cgroup and therefore exercised only the fallback to docker stats. **It has never run on this guest, and no resource series has been produced with it anywhere.** What the guest checks of 3.4 do settle (sealed record of 2026-09-18, `docs/evidence/integrated-qemu/2026-09-18-build-boot/2026-09-18-integrated-3209b17/guest/guest-checks.txt`): `Cgroup Version: 2` and `Cgroup Driver: systemd`, so the `system.slice/docker-<id>.scope` layout the collector globs is the expected one. Unverified: that the six containers' scopes appear there with readable `cpu.stat`, `memory.current`, `memory.stat` and `memory.max`; the **cadence actually achieved** under TCG — the loop sleeps the whole interval *after* each sample, so the period is the interval plus the cost of the sample, and a cost above 4 s would again break `MAX_SAMPLE_GAP_S = 5.0`, invalidating a run for a new reason; that `--source auto`, which is evaluated once before the warm-up, finds a container cgroup at that instant rather than falling back to the slow docker-stats loop for the whole run (after a reboot, test 8, or around a `down`/`up -d`, 6.5 and test 1, the containers may not be up yet); that a name resolved once from `config.v2.json` under `/var/lib/docker` is available to the collector as the hooks run it (`sudo systemd-run`, i.e. as root — an unprivileged collector falls back to a `docker ps` listing and then to the 12-character id, and a name that fails to resolve once is cached for the life of the state file, which would split one container into two series); that `journalctl -u egw-resources-<run_id>` still holds the collector's source line after `systemd-run --collect` has garbage-collected the unit; and busybox `awk` itself, which the adversarial review of 2026-09-19 could not run on the workstation (the whole path was run there under `sh`, `dash` and `bash` with `awk` forced to `gawk --posix` and `gawk --traditional`, nine combinations, identical rows and no diagnostic — evidence of portability, not of busybox). `shellcheck` was not available on the workstation; the error-level gate runs in CI. The consequence for this runbook is stated in the Status paragraph and under test 1: the harness parts of tests 1 and 6 have not been re-run, the battery is not complete and nothing is measured.
