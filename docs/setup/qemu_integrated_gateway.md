...
host$ T6=stop; if [ -n "$SEED" ] && wait_ready && drained && $REC snap --prefix $P/$RID --label before --seed $SEED; then harness_run $RID --restart-cmd "$RESTART" --restart-at-s 300 --allow-missing-restart-evidence && T6=ok; else stop "test 6: no seed, not ready, not drained or no 'before' snapshot - the harness run was NOT started"; fi     # the plan's derived seed, not 42: these are other devices; the before/after twin snapshots and the drain are taken by the helpers around this line, outside the harness, and the harness records that exception as a deviation (ADR 0011, item 18: a controller_restart run without --twin-snapshot-cmd, --drain-cmd and --post-drain-fetch-cmd is otherwise invalid; the proof's session driver hands the harness its own templates)
