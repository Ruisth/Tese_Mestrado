...
> requires. The supervisors approved nothing, no gate is closed and no claim is
> admitted. Current state remains in `PROGRESS.md`.
...
|---|---|
| Governance publication | This documentation change, in parallel with ongoing bounded tests. It confirms consistency only: no gate, no claim, no supervisor approval |
...
| Nine integration/recovery test families | Target 2026-09-21 to 2026-09-24; report per-test evidence, failures and residual risks. **None has been run** |
...
- Supervisor silence is not approval. D001 and D004 block the final academic
  release; D007 blocks `exp-v1`; D011 (integrated title and RQ wording) and
  D014 (the evaluation scope and any academic use of emulated results) also
  block the final academic release. The student's adoption of the plan settles
  execution, not these.
...
**Cut rule:** obtain the supervisor decisions without treating silence as
approval. The required result is an alignment request covering D001–D014 and
versioned authority/provenance records. The university ARM64 request and the
48-hour public-host fallback are **deferred with the native route** and are no
longer conditions of this gate.

| Action | Expected evidence | Dependencies |
|---|---|---|
| Obtain explicit validation of scope, title, objective, RQs and the integrated architecture | Supervisor reply and dated D001/D011 decisions linked from `PROGRESS.md`; [`scope_and_rqs.md`](scope_and_rqs.md) aligned with the decision. The student's adoption of plan v2.0 is not that validation | Supervisors |
| Send the consolidated alignment email with Chapter 2 and the D001–D014 request, using the revised memo | Sent-message copy/date; `sent_at` and response references in the canonical decision log. Include the question of the evaluation scope and any academic use of emulated results (D014); a reply is needed before the evaluation chapter is written | Student action; [`supervisor_email_g0.md`](supervisor_email_g0.md) and the [revised memo draft](../governance/proposals/supervisor_alignment_memo_v2.0_proposal.md); Chapter 2 attachment |
...
| Run the nine integration/recovery test families of the runbook | Per-test evidence, retained failures and a residual-risk list; **none has been run** | Stable deployment in the guest |
...
| Execute every mandatory scenario, including nominal, load, invalid payload, dropout/reconnect, restart and a bounded stability run if one is defined at the pilot | One write-once manifest/evidence set per scenario and a demonstrated real disconnection/recovery path. The 24-hour soak is not transferred to the emulated environment and becomes an explicit limitation | Integrated emulated guest |
...
| Execute the bounded pilot | Pilot raw data, manifests, environment records and checksums for every condition; pilot data marked non-citable. Select the emulated functional campaign here: run identities, repeats, durations, load levels and acceptance criteria are chosen prospectively and never inherited from the native plan | Integrated emulated guest; corrected harness |
...
verified checksums. Never the 95 runs and never the 24-hour soak under
emulation: those become documented limitations. An optional condition may be
cut only as a documented limitation; never fill an evidence gap with an
unsupported conclusion.

| Action | Expected evidence | Dependencies |
|---|---|---|
| Execute the frozen **emulated functional campaign** selected at the pilot, covering functional, cold-start, twin-creation, nominal and fault/reconnect/restart conditions | `experiments/results/raw/<run_id>/` for every predefined identity, each with manifest, environment, valid measurement window and verified `SHA256SUMS`, grouped by execution mode and never pooled across modes. The 95-run campaign and the 24-hour soak are **not** carried over to the emulated environment | Integrated emulated guest; `exp-v1` |
