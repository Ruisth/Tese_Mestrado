...
| R4 | Ditto exceeds resources | Medium | High | Student | **Materialised on 2026-09-18; partly mitigated: the correction of pull request #34 (`d0c9238`) is merged, stability not demonstrated** - a memory-cgroup OOM killed the `ditto-things` JVM **during the power-off** of the first end-to-end session, at a 512 MiB container limit. Diagnosis of the same day: the three Ditto services idle at 94-95 % of that limit and reached 98.1 % after a 672-message workload, and a controlled `docker compose stop -t 60` after that workload produced no OOM anywhere in the boot. The corrective change landed separately through pull request #34: the three Ditto limits were raised from 512 MiB to 768 MiB against that measurement, and the stack is stopped with `docker compose stop -t 60` before power-off. The battery of 2026-09-18, run with that change applied, recorded no memory-cgroup OOM kill (candidate record, not admitted). These are bounded observations: general or prolonged stability is not demonstrated and is not claimed | before the integrated baseline may be called stable (target 2026-09-25) | OOM or persistent swapping; a service already near its container limit while idle | Graceful stop before power-off; size the container memory limits against the observed usage; keep non-essential services removed. The guest's 8 GiB is an emulator setting, not a demonstrated equivalence to a physical gateway |
...
## Requirements and platform-selection risks added on 2026-09-21

These qualitative assessments support plan v2.1; no new implementation scope
or claim follows from them. Rui owns the corresponding actions.

| ID | Risk / likelihood and impact | Trigger | Mitigation and associated deliverable |
|---|---|---|---|
| RS01 | Documentary board compatibility is mistaken for a validated gateway; medium likelihood, high academic impact | A board, RAM minimum or physical performance claim appears without native evidence | Label the Pi 5 choice provisional; retain BSP/ISA checks and unverified properties in SUP-01/SUP-02/SUP-07 |
| RS02 | Hardware comparison expands into procurement/porting and displaces the first demonstrator; medium likelihood, high schedule impact | Selection review exceeds the one-to-two-day preparation time box or native work becomes an assumed dependency | Retain the initial comparison with open questions; separate any future execution/resource decision; continue QEMU integration in parallel |
| RS03 | Revised example RQs or requirements silently overwrite the reported academic agreement; medium likelihood, high scope impact | New RQ numbering, native claims or relaxed numeric criteria enter the manuscript/protocol without a recorded aligned revision | Use SUP-04 crosswalk, retain current wording and D007 boundary, record exact agreed changes prospectively |
