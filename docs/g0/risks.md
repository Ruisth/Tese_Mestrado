...
Updated 2026-09-19 (SUT resource sampling): R18, R22 and RA6 carry dated notes
on the change of the SUT collector's sampling source, and **R34** was added for
the new sampling path, which has never run on a system under test. No state was
closed, no gate moved and nothing was measured; the earlier text of those three
rows is kept where it stands.
...
| R18 | Resources measured on the wrong host (`docker stats` runs by default on the local Docker of the harness host) | High | Critical | Agent | Mitigated at M2 on 2026-08-08 — collector on the VM, two manifests, mandatory host provenance; live proof pending. **2026-09-19:** the risk's parenthetical no longer names the SUT collector's default path — since that date `src/deployment/scripts/collect-resources.sh` reads the SUT's own cgroup v2 accounting under `/sys/fs/cgroup` and calls no daemon, with `--source docker` restoring the previous `docker stats` loop. The collector still runs on the SUT, the mandatory `host` column and the two manifests are unchanged, and the harness's own `--local-resources` sampler is still the opt-in, dev-only one. The new path has never run on a SUT, so this row's "live proof pending" stands and the sampling source itself is now a risk of its own (R34) | before G4 (2026-09-06) | A pilot without unambiguous identification of the VM | Collector on the VM, or an explicit and recorded remote channel, + two manifests (`sut_environment` on the VM, `loadgen_environment` on the simulator host) |
...
| R22 | Warm-up contaminates the resource metrics (the sampler covers warm-up + run; the analysis does not filter the measured window) | High | High | Agent | Mitigated at M2 on 2026-08-08 — measured window in the manifest, filter in the analysis and cadence caps. **2026-09-19:** the measured-window filter and the caps are unchanged (`MAX_SAMPLE_GAP_S = 5.0`, `RESOURCE_WINDOW_COVERAGE_MIN_FRAC = 0.90`), but what the collector delivers into them changed: its loop sleeps the interval **after** each sample, so the achieved period is the interval plus the cost of one sample, with no compensation, and the first sample of a fresh state file only primes the CPU delta and writes no row, so a series begins one interval after the collector starts. Started before the warm-up and stopped after the confirmation window, as the harness hooks do, both effects fall outside the measured window; neither has been measured on the emulated guest (R34) | before G4 (2026-09-06) | `resources.csv` includes the pre-measurement period; or, since 2026-09-19, a series whose first sample falls inside the measured window because the collector was started with it | Record `measured_started_utc` in the manifest and filter the measured window in the analysis; start the collector at least one interval before the warm-up |
...
| RA6 | Local resources treated as ARM resources | Agent | **Mitigated at M2** | Mandatory host-provenance column in `resources.csv`; SUT collection by `src/deployment/scripts/collect-resources.sh` on the VM, ingested through `run --resources-from`; the local sampler is opt-in (`--local-resources`) and is marked with `resource_source` (`local-dev` invalidates a timed run). Block P5 added semantic validation of the series at analysis time (unusable rows discarded and **counted** by reason: missing column, empty field, unreadable timestamp, non-numeric value, time going backwards), visible in `resources_rows_dropped`/`metrics_rows_dropped`. Equivalent to R18; live proof pending. **2026-09-19:** the SUT collection named above is unchanged in place and in contract, but its default source is now the SUT's cgroup v2 accounting under `/sys/fs/cgroup` rather than `docker stats` (`--source docker` restores the previous path); the host-provenance column, the opt-in local sampler and the `resource_source` mark are untouched. Two residues: the CSV records **no** sampling source — it is announced once on the collector's stderr, which under the documented `systemd-run` orchestration reaches the guest journal and never the harness — and the collector's synthetic-tree options can point its readings at any directory, so a file produced that way would pass every ingest rule. Keep the collector's stderr with the run evidence and never use those options outside the test suite (R34) |
...
functional platform layer only) while D006 remains a separate pending
supervisor decision.
...
## Risk added on 2026-09-19 (SUT resource sampling) — R34

On 2026-09-19 `src/deployment/scripts/collect-resources.sh` stopped calling
`docker stats --no-stream` for every sample and began reading the SUT's own
cgroup v2 accounting under `/sys/fs/cgroup` (`--source docker` restores the
previous loop). The reason was measured, but on the emulated guest only and
outside this repository: each `docker stats` call cost 3 to 5 s idle and about
21 s while the stack was working under QEMU/TCG, so the collector produced 7
distinct sample instants across 42 rows in a 150 s window against the ingest
minima of 30 rows and 30 distinct instants, and the harness correctly marked
the timed runs `invalid` and withheld `SHA256SUMS`. The **harness** was right;
the **sampler** was the defect. Replacing an evidence producer creates a risk
of its own, and this row carries it. The rewritten path is covered by
`src/tests/test_collect_resources.py` (11 scenarios against a synthetic cgroup
tree under `sh`, `dash` and `bash`; `33 passed` on WSL2 Ubuntu-24.04) and by
nothing else: it has never run on a system under test, the harness parts of
integration tests 1 and 6 have not been re-run, and **nothing is measured**.

| ID | Risk | Prob. | Impact | Owner | State | Deadline | Early signal | Mitigation |
|---|---|---|---|---|---|---|---|---|
| **R34** | **The rewritten resource sampling path is unproven on the SUT, and the CSV it produces records no sampling source.** Six named exposures, all from the review of 2026-09-19: (a) the loop sleeps the interval **after** the sample, so the period is the interval plus the sample's cost, uncompensated and unmeasured on the guest — a cost above 4 s puts every consecutive pair beyond `MAX_SAMPLE_GAP_S = 5.0` and invalidates the run again, from a new cause; (b) `--source auto` is decided once, before the warm-up, so a collector started while the containers are still coming up selects the slow docker-stats loop for the whole run and says so only on stderr; (c) the CSV carries no provenance of the source or of the roots, and the collector's synthetic-tree options (`--cgroup-root`, `--docker-root`, `--uptime-from`, `--meminfo-from`, `--state-file`) let it read its readings from any directory, so such a file is indistinguishable at ingestion from a measured one; (d) a container name that fails to resolve once is cached, the 12-character-id fallback included, which can split one container into two series and fail the per-container ingest rules; (e) an unreadable `memory.stat` or a missing `MemTotal` substitutes zero rather than skipping the row, and a zero passes the numeric gate as if it were a measurement; (f) nothing prevents two collectors from appending to one CSV | High while unproven | **Critical for RQ3** (the collector is the sole evidence producer for C09 and feeds the sustained-window rules of C08 and C13) | Agent | **Open** — the change exists and is unit-tested against a synthetic tree; no run on a SUT, no measured series, no gate closed, no claim admitted. Exposures (a) to (f) are recorded, not fixed: they live in `src/`, which the documentation change of 2026-09-19 does not reopen | Re-run of the harness parts of integration tests 1 and 6 on the emulated guest, before any pilot run of Section 8 of [`../setup/qemu_integrated_gateway.md`](../setup/qemu_integrated_gateway.md) | A `resources.csv` with far fewer distinct instants than the run has seconds; a run invalidated for sampling gaps while the stack was healthy; one container appearing under both a name and a 12-character id in the same file; `mem_pct` exactly `0.00` beside a non-zero `cpu_pct`; the collector's stderr saying `falling back to docker stats` | The re-run of tests 1 and 6 is the acceptance step and it decides this row; until then no resource series is evidence. Keep the collector's stderr (the `collecting container resources from <source>` line, in the guest journal under `systemd-run`) with the run evidence, and treat a series whose source cannot be established as missing rather than as measured. Never use the synthetic-tree options outside `src/tests/test_collect_resources.py`. Exposures (a) to (f) are a separate code change, to be decided before `exp-v1`; none of them may be described as mitigated on the strength of the synthetic-tree tests |
