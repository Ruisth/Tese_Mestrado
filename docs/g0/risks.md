...
| R25 | Loss of the local repository (zero remotes, zero tags, workspace on Nextcloud) | Medium | Critical | Both | **Mitigated** — on 2026-08-10 the repository and the `backups/` bundles were copied out of the synchronised Nextcloud folder to a private location, so the "bundle in the same synchronisation domain" condition no longer holds. **A private remote was created on 2026-08-11** (`Ruisth/Tese_Mestrado`, private, with a ruleset requiring pull requests on `main` and `dev`): the repository now exists in a failure domain independent of the local disk. `refs/tags` remains empty, by choice — tags stay reserved for real evidence | met on 2026-08-11 | Repository and bundle on the same physical medium; absence of a remote | Private remote (preferred option, done) or a verified Git bundle outside Nextcloud (done); tags reserved for real evidence (`g1`, `g2`, `exp-v1`, `data-v1`, `rc1`), never anticipated |
