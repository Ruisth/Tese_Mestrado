...
| R25 | Loss of repository or evidence provenance | Medium | Critical | Both | **Mitigating** — the private remote exists; nine local bundles verify; exact-tree rewrite mappings are recorded; protected tags `evidence/g1-yocto-build-5770c0a` and `evidence/g1-bringup-driver-367c929` preserve the exact preliminary build and its driver lineage. `egw-20260814-post-pr21-merge.bundle` is a complete-history snapshot with `dev` at the PR #21 merge commit `f1ce8d9` (SHA-256 `116d59eb…`). No verified off-machine copy is recorded — the deadline of "before G1 acceptance" was missed: G1 was accepted on 2026-08-14 with the copy still outstanding, which sharpens rather than retires this risk | off-machine copy — immediate | All bundles remain on the same physical machine | Copy the current bundle off-machine, verify it there and record its SHA-256/location without publishing restricted history |
