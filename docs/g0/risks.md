...
| R7 | Late feedback | Medium | High | Student | Open — the alignment email exists only as a draft and no contact attempt is recorded | send immediately with the revised memo; record every contact attempt | No reply to the recorded alignment request | Deliver Chapter 2 and the D001–D014 request early, record every contact attempt and continue only with reversible work; silence is never approval for D001, D004, D007, D011 or D014 |
...
|---|---|---|---|---|
| RA1 | Email/rules/VM/WSL2 delay G0–G2 | Student | **Open — student action** | Largely overtaken by events: WSL2 with Ubuntu 24.04.4 LTS is installed and operational on ext4 and built both images (see R30), and since 2026-09-18 the integrated emulated guest is the adopted environment, so the absent native ARM64 VM blocks no gate (see R28). What remains is the unsent alignment email and the unanswered D001–D014. Overlaps R10 and R14; no agent action closes it |
...
| **Academic use of emulated results is not agreed** | R7, R14 and decision D014 | Open and outside the student's control. Emulated functional runs need no agreement to exist, but presenting them as the evaluation of the dissertation does. Ask the question explicitly in the alignment package; silence is never approval, and the evaluation chapter is not written against an assumed answer |
| **The schedule has no slack and no measured effort basis** | R8, R14 and RA15 (materialised) | The submission target of 2026-10-20 and the final deadline of 2026-10-31 rest on a first-party statement; the 225-345 hour estimate of 2026-09-16 is superseded pending the student's re-estimate, and `actual_h`, `remaining_h` and `forecast` remain empty, so no completion forecast exists. Apply the reforecast triggers of the adopted plan rather than moving a date silently |
...
milestone date is a planning target. D004 and the submission and front-matter
rules are included in the draft alignment email, but there is no recorded send
or supervisor reply. The cut rules therefore reduce scope or record a
...
supervisor decision. The 2026-09-30 baseline and the 2026-09-29 internal cut-off
