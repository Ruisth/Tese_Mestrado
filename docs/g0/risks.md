...
| **Academic use of emulated results is not agreed** | R7, R14 and decision D014 | Open and outside the student's control. The student reports supervisor approval to proceed with the QEMU tests and approval of RQ3 evaluated in QEMU; **approving the route is not approving the academic use of its results**, and neither report is a documented supervisor decision. Emulated functional runs need no agreement to exist, but presenting them as the evaluation of the dissertation does. Ask that question explicitly in the reduced alignment package; silence is never approval, and the evaluation chapter is not written against an assumed answer |
