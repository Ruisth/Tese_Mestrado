...
2026-08-07 and still carried it on 2026-08-10. On 2026-08-11 gate G1 produced
sealed evidence and two rows moved: **C01** to partial (the build is
evidenced; the rebuild from an independent clean checkout is pending and
belongs to G4) and **C02** to bring-up evidence produced (two boots driven by
the automated driver; the campaign's five `qemu_boots` runs are still to be
executed). The other **13 claims remain `Pending — no evidence`**.

**0 of the 15 claims are accepted**, and **no gate has been accepted**: G1
stays in progress — evidence produced, acceptance pending.
`experiments/results/raw/` is still empty; the G1 evidence lives in
`docs/evidence/g1-yocto-qemu/`, sealed with `SHA256SUMS` that verify from a
clean clone. The revisions of 2026-08-08 and 2026-08-10 made the method
columns more demanding; **neither of them produced evidence**.
...
|---|---|---|---|---|---|---|---|---|
| C01 | The `egw-image` image is built reproducibly from a `kas` manifest with exact tags/commits pinned (Yocto 5.0.19/Scarthgap), from a documented clean checkout | 4 | RQ1 | BitBake build from a clean checkout; full build log; checksums of the artefacts | `docs/evidence/g1-yocto-qemu/kas-checkout.log`, `kas-build.log`, `image-packages.manifest`, `SHA256SUMS`; image sha256 `9e9b8e8c…`; kernel sha256 `4457ef38…` | (n/a — verification by log and checksum) | `src/yocto/kas/egw-qemuarm64.yml` @ `5770c0a` | **Partial** — build of 5715 tasks, all successful, logged and sealed (2026-08-11). **The rebuild from an independent clean checkout is still pending and belongs to G4**; until then the claim cannot be treated as demonstrated. Gate G1 acceptance: pending |
| C02 | The `egw-image` boots on `qemuarm64` through to systemd with networking and a working OCI runtime, repeatably (2 bring-up boots at G1; 5 boots of the `qemu_boots` condition in the final campaign), including running a test container | 4 and 5 | RQ1 | **Two distinct, non-interchangeable sets:** (a) G1 bring-up (plan §7.1) — 2 boots produced by the automated driver `src/yocto/scripts/boot_check.py` at commit `32f6604`, which logs in over the serial console, runs a fixed check list and decides the outcome from the output; they are the gate G1 pass criterion and were **NOT executed by hand**; (b) final campaign — `qemu_boots` condition of the frozen protocol, 5 boots ingested by `run --external-timings`. In both: a test container is run inside the guest and **ZERO** performance inference is drawn (plan §5.1) | `docs/evidence/g1-yocto-qemu/boot1.log`, `boot1.result.json`, `boot2.log`, `boot2.result.json`, `SHA256SUMS` | `src/yocto/scripts/boot_check.py` @ `32f6604` | `build/g1-yocto-qemu` @ `5770c0a` | **Bring-up evidence produced (2026-08-11)** — two boots, each with **6 of 6 required assertions passed, 3 of 3 supplementary observations recorded and a clean power-down confirmed**, by the driver `src/yocto/scripts/boot_check.py` at commit `32f6604`. The observations assert nothing and are not counted as verification. **The 5 boots of the campaign `qemu_boots` condition are still to be executed.** Gate G1 acceptance: pending |
