...
- **P-10** — the 50-minute rule is measured from the instant taken
  ...
  reached. (Until these corrections the rule reached during `pre`, and
  then an allowance spent before `pre` was dispatched, ended the attempt
  `not-run`; the tests that expected it encoded that wrong rule and are
  corrected.)
