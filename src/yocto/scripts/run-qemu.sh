...
# (plan section 5.1).
#
# Gate G1 "two boots" rule (plan sections 8 and 8.1): the image must boot
# twice, reach systemd multi-user, bring networking up and execute one
# container. Run this script twice with distinct names so each boot leaves
# its own complete console log:
#   ./scripts/run-qemu.sh boot1
...
#   systemctl is-system-running        # expect "running" (justify "degraded")
...
# Interactive stdin stays attached to the terminal; stdout/stderr are teed so
# the full session lands in the evidence log.
