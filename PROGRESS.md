...
`docs/governance/proposals/`, plan v1.2 is back, unmodified, at its canonical
path, and the proposal section carries the dates confirmed by the student
(LOG #C031).
...
Addendum 2026-09-19, SUT resource instrumentation only: the collector
`src/deployment/scripts/collect-resources.sh` now reads the kernel's cgroup v2
accounting instead of calling `docker stats` once per sample (LOG `#C034`).
The sampler, not the harness, was the reason the timed run of 2026-09-18
reached 7 distinct sample instants in a 150 s window against a minimum of 30
and was marked `invalid`. Of the nine integration/recovery test families
exercised in the emulated guest on 2026-09-18, seven passed and tests 1 and 6
have a failing harness part (the ad-hoc part of test 1 passed); those results
are candidate evidence of that session, held outside this repository and
unsealed. The defect is corrected in this change and **unproven on the guest**:
the collector was exercised only against a synthetic cgroup tree on WSL2
(33 cases under `sh`, `dash` and `bash`), nothing ran on the emulated guest,
nothing was measured, and the harness parts of tests 1 and 6 have not been
re-run — that re-run is the acceptance step. A review of the change recorded
open defects, first among them that the sampling period is the interval plus
the cost of a sample, so the cadence the collector achieves on the guest is
unknown; the actions are in [`docs/g0/backlog.md`](docs/g0/backlog.md). The
addendum changes no gate, claim, maturity level or sealed test figure.
...
## Proposed scope and forecast — plan v2.0 proposal (2026-09-16, published as a proposal 2026-09-18)
...
The [plan v2.0 proposal](docs/governance/proposals/INTEGRATED_DEVELOPMENT_PLAN_2026_v2.0_proposal.md)
and [ADR 0008](docs/adr/0008-integrated-yocto-arm64-evaluation.md) — *Proposed —
...
service host. They are a proposal, kept under
[`docs/governance/proposals/`](docs/governance/proposals/README.md); merging
them adopts nothing.
[Plan v1.2](docs/governance/INTEGRATED_DEVELOPMENT_PLAN_2026.md), unmodified at
its canonical path, remains the plan in force until the student decides after
consulting the supervisors. Nothing has been sent to the supervisors (D001–D014 are all
...
| Integrated QEMU/TCG profile (ARM64 emulated on the x86-64 WSL2 workstation) | Ordered first by the project review once no native ARM64 virtual machine could be obtained on 2026-09-17. In pull request #28 (merged into `dev` on 2026-09-18) the integrated image was built (commit `03e333e`) and booted twice under QEMU/TCG (commit `3209b17`) on 2026-09-18 with every build and boot acceptance check passing. The same day an isolated MongoDB 7.0.39 test passed on that guest (start, write/read, restart, persistence across a guest power cycle); the build, boot and MongoDB records were sealed as technical evidence in `docs/evidence/integrated-qemu/` (pull request #29, merged into `dev` on 2026-09-18). Pull requests #32 (broker secret hand-over and ACL probe) and #31 (controller progress counters) were merged into `dev` the same day; merging validates no stack and accepts no gate. On 2026-09-18 the six-container stack was deployed inside the guest and the nine integration/recovery test families of runbook Section 7 were exercised there: **seven passed, and tests 1 and 6 have a failing harness part** (the ad-hoc part of test 1 passed), caused by the SUT resource sampler and not by the harness. Those results are candidate evidence of that session, held outside this repository and unsealed; the battery is not complete. The sampler defect is corrected on 2026-09-19 (LOG `#C034`, addendum above) and is **not yet proven on the guest**: the re-run of those two harness parts has not happened. Nothing has been measured; sealing is not acceptance | Six containers deployed inside the guest; functional path simulator -> MQTT/TLS -> controller -> Ditto -> API; correctness, fault handling, recovery and persistence. Functional and integration evidence only, never native ARM64 performance evidence (plan v2.0 proposal, section 10) |
...
| Services on Yocto (G2) | Controller/simulator/Compose code exists; the six services ran in the emulated guest on 2026-09-18 (candidate evidence, unsealed, emulated, and two harness parts failing), so no admitted integrated live trace and none on native ARM64 | Actual Mosquitto/controller/Ditto/MongoDB execution in the Yocto guest |
...
| Effort and dates (proposal) | Estimated 225-345 active hours remaining (estimate of 2026-09-16, not re-estimated); 8 h/day reported. Planned submission 2026-10-20 and final delivery deadline 2026-10-31, confirmed by the student on 2026-09-18 after discussing the dates with the supervisors (a first-party statement; no administrative document checked); 2026-10-21 to 2026-10-31 is a contingency window for essential corrections only | Capacity from 2026-09-19 to 2026-10-20 at 8 h/day is 256 h, which covers only the lower part of the range; the milestone dates of the proposal (section 6) are forecasts made on 2026-09-18, none is completed; logged hours and a re-estimate by the student |
...
accepted by this proposal or by the emulated results. The proposed milestones
and reforecast triggers are in sections 6-7 of the plan v2.0 proposal; they
were reforecast on 2026-09-18 backwards from the planned submission of
2026-10-20 and are forecasts, not completed milestones.
...
| Integrated QEMU/TCG gateway profile (`kas/egw-qemuarm64-integrated.yml`, `egw-gateway-image`, `egw-gateway-config`, `scripts/build-profile.sh`, `scripts/run-qemu-integrated.sh`, runbook `docs/setup/qemu_integrated_gateway.md`) | built and booted once under QEMU/TCG (2026-09-18) — **the six-container stack was deployed in the guest on 2026-09-18 and the nine integration test families were exercised there: seven passed, tests 1 and 6 have a failing harness part (SUT resource sampler, corrected 2026-09-19 and not yet re-run); those results are candidate evidence held outside this repository and unsealed. The sealed capsule (row below) covers the build, the boots and the isolated MongoDB test only, and sealing is not acceptance** | build from commit `03e333e` (5,556 tasks) after a first attempt at `68f9ae7` failed on an RPM directory-mode conflict; two boots from commit `3209b17` after a first boot attempt failed in the wrapper's `runqemu` invocation; every artefact and guest acceptance check of runbook 2.4 and 3.4 passed (Cortex-A76 model, 4 vCPUs, 8 GiB, data disk on `/var/lib/docker`, Compose v2.26.0, key-only SSH, persistence across boots); kernel `Image` byte-identical to G1 although its tasks were re-executed; G1 build tree and inputs untouched, the eight evidence seals verify (59 artefacts). Record: `docs/reviews/2026-09-17-egw-image-audit.md` Section 13. Evidence sealed in `docs/evidence/integrated-qemu/`. ARM64 **emulated**: functional evidence only | none — follows the plan v2.0 proposal (`docs/governance/proposals/`, not in force); accepts no gate | M1 |
| Evidence of the integrated QEMU/TCG profile (pull request #28): build, two boots, isolated MongoDB 7 test | sealed 2026-09-18 — **functional only, emulated; the capsule was taken before the stack was deployed and covers the build, the boots and the isolated MongoDB test** | integration (emulated) — `docs/evidence/integrated-qemu/`: image built at `03e333e` after a preserved failed attempt at `68f9ae7`, two boots at `3209b17` with every build and guest acceptance check passing (Cortex-A76 model, 4 vCPUs, 8 GiB, data disk on `/var/lib/docker`, Docker 25.0.9 with Compose v2.26.0, key-only SSH, persistence across boots), kernel `Image` byte-identical to G1, G1 build tree unchanged; MongoDB 7.0.39 pulled by digest inside the guest: start, write/read, restart, recreation and persistence across a guest power cycle, 35 checks, 0 failed. Manifests verified by `tools/ci/verify_evidence.py` | none — sealing is not acceptance; no gate, no claim | M2 |
...
| Broker secret hand-over and ACL probe (`scripts/prepare-broker-secrets.sh`, `scripts/probe-acl.sh`, step 3b of `validate-config.sh`) | written — **never run against a Docker engine or a broker** | unit (stubs) — `src/tests/test_probe_acl_verdict.py`: 94 cases of the probe's verdict logic under dash and bash with a stub `docker` (Linux only; skipped on Windows); `sh -n` on every script. Mosquitto 2 opens `password_file` and `keyfile` after dropping to uid 1883, so both are handed to that uid with mode 0600 and the read is proved in a one-shot container; the probe proves the ACL with known traffic and answers PASS, FAIL or INCONCLUSIVE, never PASS on an execution error or an incomplete broker-log collection | none — accepts no gate | M2 |
| Prebuilt controller image in the deployment and versioned reconciliation helper (`compose.yaml` without `build:` and with `pull_policy: never`, `scripts/build-controller-image.sh`, `scripts/verify-controller-image.sh`, step 7 of `validate-config.sh`, `src/egw_experiments/itest_reconcile.py`; LOG `#C030`) | written 2026-09-18 — **nothing built, loaded, pulled or run on the guest yet**; the controller image has **unlocked Python dependencies** (`pip install .`), to be resolved before the experimental freeze | unit (stubs and fakes) — `src/tests/test_deployment_prebuilt_controller.py` (35 cases: 7 read the compose decision as text on every platform, 28 run both scripts with a stub `docker` under dash and bash on Linux) and `src/tests/test_experiments_itest_reconcile.py` (106 cases, fakes only, unmodified `compute_run_metrics`, the confirmation window imported and never redefined, every `$REC` line extracted from the runbook and parsed); `docker compose config` accepts the file and the resolved model differs from the previous one only in the controller's `build`/`pull_policy`; runbook Sections 4 to 7 follow the route (five external images pulled by pinned digest inside the guest, controller loaded from a checksummed archive and compared with its identity record, the deployment tree and the harness checked out at that image's commit). ShellCheck not run locally | none — accepts no gate | M2 |
| SUT resource collector on cgroup v2 (`src/deployment/scripts/collect-resources.sh`, `src/tests/test_collect_resources.py`; LOG `#C034`) | written 2026-09-19 — **never run on the emulated guest**; the sampling source and roots of a run are recorded in neither the CSV nor the manifest, and the cadence achieved on the guest is unknown (the period is the interval plus the cost of a sample) | unit (synthetic tree) — 33 cases (11 scenarios x 3 shells: `sh`, `dash`, `bash`) on WSL2 Ubuntu 24.04 against a synthetic `/sys/fs/cgroup`, Docker data root, `/proc/uptime` and `/proc/meminfo`: exact CPU and memory arithmetic on docker's cgroup v2 definitions, both cgroup driver layouts, the name fallback chain, a container that disappears, a counter that goes backwards, a refused foreign header, and a five-sample cadence case whose output passes `validate_resources_csv` with only the two threshold arguments relaxed to the sample count. A review of the change recorded open defects (cadence compensation, sampling-source provenance, the one-shot `auto` decision, a cached name fallback, a zero substituted for a missing input, two collectors on one CSV, the state file rewritten in place). ShellCheck not run locally; no container, no Docker engine and no measurement | none — accepts no gate; the acceptance step is the re-run of the harness parts of integration tests 1 and 6 in the guest | M2 |
| MQTT→Ditto controller | yes | unit — tests with fakes (part of the current sealed suite of 701 tests); the progress counters of `GET /metrics` added on 2026-09-18 (`received`, `in_progress`, `processing_errors`, ADR 0010) are unit-verified by tests **outside the sealed suite** — accounting identity, single-snapshot reading, fault and cancellation paths, all with fakes; no real MQTT/Ditto, no real restart, no ARM64 | G2 — Pending | M2 |
